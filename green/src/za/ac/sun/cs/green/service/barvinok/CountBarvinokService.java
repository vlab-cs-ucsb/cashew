...
public class CountBarvinokService extends CountService {
	...
	private class HMatrix {
		...
		private Map<IntVariable, BigInteger> variableRange;
		...
		public HMatrix() {
			...
			variableRange = new HashMap<IntVariable, BigInteger>();
		}
		 ...
		 */
		public BigInteger count(Expression expression) {
			explore((Operation) expression);
			for (IntVariable v : allVariables) {
				BigInteger x = BigInteger.valueOf(v.getUpperBound());
				variableRange.put(v, x.subtract(BigInteger.valueOf(v.getLowerBound())));
			}
		 ...
		 * @return the number of satisfying solutions as an {@link BigInteger}
		 */
		private BigInteger processInput(String input) {
			if (store == null) {
				return new BigInteger(invokeBarvinok(input)).multiply(correction);
			} else {
				...
				return new BigInteger(count).multiply(correction);
			}
		}
