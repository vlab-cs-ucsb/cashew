...
public abstract class BasicService implements Service {
	...
	/**
	 * Returns the reporting prefix, which consists of the name of the root service
	 * that this (sub)service instance is associated with, followed by a single colon
	 * and finally the simple runtime class name of the service subclass. This can be
	 * used by reporters to show a prefix that not only reveals the class name, but
	 * also the name of the main toolchain that this instance of said classname is in.
	 * 
	 * (This is useful when you have multiple instances of the same service involved
	 * in different root services.)
	 * 
	 * @return the reporting prefix string for this service.
	 */
	public String getPrefix() {
		return this.getSolver().getRootServiceName(this) + ':' + this.getClass().getSimpleName();
	}
