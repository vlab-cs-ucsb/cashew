...
public class SATSlicerService extends BasicService {
	...
	public void report(Reporter reporter) {
		...
		reporter.report(getPrefix(), "invocations = " + ic);
		reporter.report(getPrefix(), "totalConjuncts = " + tc);
		reporter.report(getPrefix(), "minimalConjuncts = " + mc);
		reporter.report(getPrefix(), "conjunctReduction = " + ((tc - mc) * 100.0D / tc));
		reporter.report(getPrefix(), "totalVariables = " + tv);
		reporter.report(getPrefix(), "minimalVariables = " + mv);
		reporter.report(getPrefix(), "variableReduction = " + ((tv - mv) * 100.0D / tv));
	}
