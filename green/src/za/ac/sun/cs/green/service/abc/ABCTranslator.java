...
public class ABCTranslator extends Visitor{
	...
	public StringPathCondition translate(za.ac.sun.cs.green.expr.Expression expression){
		try{
		...
		//check if stack is not empty. If so, add to constraints!
		...
		if (constraints!= null && constraints.size() > 0){
			StringConstraint temp = constraints.remove(0);
			StringConstraint new_spc = temp;
			...
			if (constraints_numeric!= null && constraints_numeric.size()>0){
				Constraint temp_n = constraints_numeric.remove(0);
				Constraint new_npc = temp_n;
				for (Constraint c: constraints_numeric){
					new_npc = c;
					new_npc.and = temp_n;
					temp_n = new_npc;
				}
	...
	public void postVisit(Operation operation) {
		...
		int arity = 0;
		for (za.ac.sun.cs.green.expr.Expression operand: operation.getOperands()){
			arity++;
		}
		...
		switch (o) {
			...
			e = int_exprs.pop();
			d = int_exprs.pop();
			ne = new BinaryLinearIntegerExpression(d, Operator.MINUS, e);
			int_exprs.push(ne);
			break;
			case MUL:
			recent = true; 
			...
			e = int_exprs.pop();
			d = int_exprs.pop();
			ne = new BinaryLinearIntegerExpression(d, Operator.DIV, e);
			int_exprs.push(ne);
			break;
			case CONCAT:
			recent = false; 
