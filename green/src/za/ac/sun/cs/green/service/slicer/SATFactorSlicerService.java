...
public class SATFactorSlicerService extends BasicService {
	...
	public void report(Reporter reporter) {
		...
		reporter.report(getPrefix(), "totalConjuncts = " + totalConjunctCount);
		reporter.report(getPrefix(), "minimalConjuncts = " + minimalConjunctCount);
		reporter.report(getPrefix(), "conjunctReduction = " + ((totalConjunctCount - minimalConjunctCount) * 100.0D / totalConjunctCount));
		reporter.report(getPrefix(), "totalVariables = " + totalVariableCount);
		reporter.report(getPrefix(), "minimalVariables = " + minimalVariableCount);
		reporter.report(getPrefix(), "variableReduction = " + ((totalVariableCount - minimalVariableCount) * 100.0D / totalVariableCount));
	}
