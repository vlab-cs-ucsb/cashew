...
public class Configuration {
	...
	public void configure() {
		...
		//
		// HACK: Read a few ABC-specific properties from config space
		// and store the values in the Green solver.
		// This is used by ABCCountService.
		//
		
		p = properties.getProperty("model_count.mode");
		if (p != null) {
			String mode = p.trim();
			solver.setModelCountMode(mode);
			log.info("model_count.mode: " + mode);
		}

		p = properties.getProperty("model_count.string_length_bound");
		if (p != null) {
			int bound = Integer.valueOf(p);
			solver.setModelCountStringLengthBound(bound);
			log.info("model_count.string_length_bound: " + bound);
		}

		p = properties.getProperty("model_count.vars");
		if (p != null) {
			Set<String> varNames = new HashSet<String>(); // this is just for logging
			for (String s : p.split(",")) {
				String varName = s.trim();
				if(! varName.equals("")) {
					solver.addToModelCountVars(varName);
					varNames.add(varName);
				}
			}
			log.info("model_count.vars: " +varNames.toString());
		}
	}
	...
	private void configure(String rootName, Service service, ParseTree parseTree) {
		solver.setRootServiceName(service, rootName);
