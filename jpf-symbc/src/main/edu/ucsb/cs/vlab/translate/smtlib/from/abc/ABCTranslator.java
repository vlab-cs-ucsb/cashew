	...
	*/	
	...
	public static class StringConstraints extends StringConstraintTranslator {
		...
		public void init() {
			...
			map(StringComparator.STARTSWITH, "(begins");
			map(StringComparator.NOTSTARTSWITH, "(not (begins");
			map(StringComparator.ENDSWITH, "(ends");
			map(StringComparator.NOTENDSWITH, "(not (ends");
